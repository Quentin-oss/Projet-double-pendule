...
ax1.plot(x22, y22, label = "traj m21")
...
#graph th1 th2
fig, ax=plt.subplots()
ax.plot(t,solution[:,0], label = "theta 1")
ax.plot(t,solution[:,1], label = "theta 2")
#ax.plot(t,solution2[:,1], label = "theta 2 modified")
plt.xlabel("temp")
...
for i in range(N+1):
    for j in range(i + 1, N):
        if np.abs(data[i] - data[j]) < eps: #histoir de seulement calculer les dx interessant (sinon temps de calcul bcp trop long)
            for k in range(min(N - i, N - j)): # k pour k eme dx
                dx[k].append(log(np.abs(data[i+k] + data[j+k])))
